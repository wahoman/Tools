...
class Worker(QThread):
    ...
    def __init__(self, input_root, selected_classes, db_path, output_path):
        super().__init__()
    ...
    def run(self):
        try:
            ...
            # filename_pattern = re.compile(r'_(\d{6,7})_.*?_(\d+)\.png$', re.IGNORECASE)
            
            # 1. 처리할 전체 PNG 파일 목록 생성 (수정된 로직)
            ...
            for png_full_path in png_files_to_process:
                ...
                processed_count += 1
                dirpath, png_filename = os.path.split(png_full_path)
                ...
                if not source_raw_path or not os.path.exists(source_raw_path):
                    self.log.emit(f"[오류] {png_filename} -> 원본 Raw({target_raw_filename}) 파일 없음")
                    continue
                
                result = self.process_file(source_raw_path, detector_index)
                if result:
                    imgH_gpu, ze_gpu = result
                    relative_dir = os.path.relpath(dirpath, self.input_root)
                    output_dir_for_pngs = os.path.join(self.output_path, relative_dir)
                    ...
                    ok = self.save_pseudo_bgr_from_gpu(imgH_gpu, ze_gpu, output_png_path)
                    if not ok: self.log.emit(f"[저장 실패] {png_filename}")
    ...
    def load_palettes(self):
        ...
        try:
            with open("LUT_ZeffU_0908.csv", "rb") as f:
...
class MainApp(QWidget):
    ...
    def init_ui(self):
        self.setWindowTitle('Raw to PNG Converter')
    ...
    def populate_class_list(self):
        """(수정됨) train과 valid에 공통으로 있는 클래스 폴더만 목록에 추가"""
    ...
    def start_conversion(self):
        input_root, db_path, output_path = self.input_root_edit.text(), self.db_path_edit.text(), self.output_path_edit.text()
        ...
        if not all([input_root, db_path, output_path]):
            self.log_edit.appendPlainText("오류: 모든 경로를 설정해야 합니다.")
            return
        ...
        self.worker = Worker(input_root, selected_classes, db_path, output_path)
