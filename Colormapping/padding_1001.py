...
MIN_UNDERSCORES = 5
EXT = ".png"
AVG_PIXEL_ROWS = 20  # ✨ 변경: 평균 색상을 계산할 이미지 하단 영역의 높이 (픽셀)
# ===============
...
def process_one(file_path: str):
    ...
    try:
        ...
        if h < TARGET_H:
            ...
            # ✨✨✨ 핵심 변경 로직 시작 ✨✨✨
            # 1. 평균을 계산할 영역의 실제 높이를 정합니다. (이미지 높이가 20보다 작을 수 있으므로)
            rows_to_avg = min(h, AVG_PIXEL_ROWS)
            
            # 2. 이미지 하단에서 해당 높이만큼의 영역을 추출합니다.
            bottom_part = img[-rows_to_avg:, :, :]
            
            # 3. 추출된 영역의 평균 색상을 계산합니다. (BGR 순서 유지)
            avg_color = np.mean(bottom_part, axis=(0, 1))
            
            # 4. cv2 함수에 사용하기 위해 색상 값을 정수 리스트로 변환합니다.
            pad_color_value = [int(c) for c in avg_color]
            
            # 5. BORDER_CONSTANT와 계산된 평균 색상으로 패딩을 적용합니다.
            result = cv2.copyMakeBorder(img, 0, pad, 0, 0,
                                        borderType=cv2.BORDER_CONSTANT,
                                        value=pad_color_value)
            # ✨✨✨ 핵심 변경 로직 끝 ✨✨✨
...
def parse_args():
    ...
    p.add_argument("--root", type=str, default=r"D:\hgyeo\error_class_images_only",
