...
# =========================================================
# 1. 경로 및 설정 (기존 파일의 경로 유지)
# =========================================================
DEFAULT_PATHS = {
    "_szTIPRootPath": "D:/hgyeo/APIDS_TIP/78Classified_polygon_raw",
    "_szDstRootPath": "D:/hgyeo/APIDS_TIP/output",
    "_szSrcPath":     "D:/hgyeo/APIDS_TIP/APIDS Bare Bags",
...
# =========================================================
_g_config = {}
# [수정] 리스트 컴프리헨션으로 독립적인 메모리 할당 (기존: [[0]*2000]*9 버그 수정)
_g_zeffTbl = [[0] * 2000 for _ in range(9)] 
...
# =========================================================
# 3. 핵심 유틸리티 함수 (New Logic 추가)
# =========================================================

def normalize_height_uint16(img, target_h=760):
    """ [New] 배경 이미지의 높이를 target_h(760)로 맞춤 (패딩 or 크롭) """
    h, w = img.shape
    if h == target_h:
        return img
    elif h < target_h:
...
# =========================================================
# 4. GPU 처리 및 마스킹 함수 (핵심 개선 사항)
# =========================================================

def apply_polygon_mask(img_arr, label_path):
    """
    [New] 라벨 파일(txt)을 읽어서 폴리곤 영역 밖을 65535(White)로 처리
    노이즈 및 사각형 잔상 제거의 핵심 기능
    """
...
def process_class_task(task_data):
    ...
    while success_count < target_count:
        ...
        for _retry in range(500):
            ...
            try:
                # 파일명 포맷 처리 (가끔 다른 포맷이 섞여있을 수 있음)
                tip_dim = tip_info[-1].split('.')[0] # 마지막 요소가 사이즈라고 가정
                if 'x' not in tip_dim: tip_dim = tip_info[3].split('.')[0]
            ...
            # 라벨이 prefix.txt로 되어 있는 경우 처리
            if not os.path.exists(src_label_path):
                 src_label_path = os.path.join(current_tip_path, f"{tip_prefix}.txt")
            if not os.path.exists(src_label_path):
                txts = glob(os.path.join(current_tip_path, f"{tip_prefix}*.txt"))
                if txts: src_label_path = txts[0]

            if not os.path.exists(tip_le_path) or not os.path.exists(src_label_path): continue

            try:
                # ---------------------------------------------------------
                # [New] 지능형 밝기 정규화 및 마스킹 로직
                # ---------------------------------------------------------
                raw_he = np.fromfile(tip_file, dtype='uint16').reshape(-1, tw)
                raw_le = np.fromfile(tip_le_path, dtype='uint16').reshape(-1, tw)

                # 1. 배경 레벨 측정 및 스케일링 (히스토그램 스트레칭)
                bkg_val = bkg_level_cpu(raw_he)
                if bkg_val < 10000: bkg_val = 65535 # 예외 처리
                
                scale_factor = 65535.0 / bkg_val
                if scale_factor > 5.0: scale_factor = 1.0 # 과도한 증폭 방지
        ...
        if not is_success:
            ...
            if fail_consecutive > 10: 
                return f"[{class_folder}] ⚠️ 부분 완료: {success_count}장"
...
class MergeApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.settings = QSettings("MyCompany", "XRayMergeApp_Optimized")
        self.is_all_selected = True 
        self.initUI()
        self.load_last_paths()

    def initUI(self):
        self.setWindowTitle("X-ray Polygon Synthesis (Transformed Optimized)")
